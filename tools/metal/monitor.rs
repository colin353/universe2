...
impl MetalMonitor {
    ...
    pub fn binary_cache_path_for_url(&self, url: &str) -> std::path::PathBuf {
        ...
        url.hash(&mut h);
        let filename = format!("{:016x}", h.finish());
    ...
    async fn _download_binary(
        ...
        let res: hyper::Response<Incoming> = client.request(req).await.map_err(|e| {
        ...
        let bytes = res
            ...
            .await
            .map_err(|e| {
                MetalMonitorError::FailedToDownloadBinary(format!("failed to read body: {e:#?}"))
            })?
            .to_bytes();
