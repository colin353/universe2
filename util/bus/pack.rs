...
impl<'a> Pack<'a> {
    ...
    pub fn new(bytes: &'a [u8]) -> Result<Self, std::io::Error> {
        ...
        let offsets = &bytes[num_values..offset_index_start];
        let offsets_index = &bytes[offset_index_start..offset_index_end];
        if offsets.len() % 4 != 0 || offsets_index.len() % 4 != 0 {
            return Err(std::io::Error::from(std::io::ErrorKind::InvalidData));
        }
    ...
    pub fn get(&self, idx: usize) -> Option<u32> {
        ...
        let block_offset_position = if block_offset_idx > 0 {
            read_u32_at(self.offsets_index, block_offset_idx - 1).unwrap_or(0)
        } else {
            0
        };
        ...
        let offset = if offset_position > 0 && offset_position <= u32_len(self.offsets) {
            read_u32_at(self.offsets, offset_position - 1).unwrap_or(0)
        } else {
            0
        };
...
impl<'a> Iterator for PackIterator<'a> {
    ...
    fn next(&mut self) -> Option<u32> {
        ...
        if value & 128 == 0 {
            ...
            self.offset = read_u32_at(self.pack.offsets, self.offset_idx).unwrap_or(0);
            self.offset_idx += 1;
            return Some(self.offset);
        }
    }
...
impl<'a> Serialize for Pack<'a> {
    fn encode<W: std::io::Write>(&self, writer: &mut W) -> Result<usize, std::io::Error> {
        ...
        writer.write_all(self.offsets_index)?;
        let footer_size = varint::encode_reverse_varint(self.data.len() as u32, writer)?;
        Ok(self.data.len() + self.offsets.len() + self.offsets_index.len() + footer_size)
    }
}
...
fn read_u32_at(bytes: &[u8], index: usize) -> Option<u32> {
    ...
    let chunk: [u8; 4] = bytes.get(start..start + 4)?.try_into().ok()?;
    Some(u32::from_le_bytes(chunk))
}

fn u32_len(bytes: &[u8]) -> usize {
    bytes.len() / 4
}
